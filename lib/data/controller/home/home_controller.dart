...
class HomeController extends GetxController {
  HomeRepo homeRepo;
  HomeController({required this.homeRepo, bool permanent = true});
  ...
  Future<void> loadData({bool isPullRefresh = false}) async {
    ...
    final token = apiClient.initToken();
  ...
  getUserInfo() async {
    ResponseModel responseModel = await homeRepo.getUserData();
    if (responseModel.statusCode == 200) {
      ...
      if (model.data != null &&
        ...
        name = "${model.data?.user?.firstname} ";
        // ${model.data?.user?.lastname}";
        provider = model.data?.user?.provider ?? "";
      }
    } else {
