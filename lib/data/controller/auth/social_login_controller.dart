...
import '../../../view/packages/signin_with_linkdin/signin_with_linkedin.dart';
...
class SocialLoginController extends GetxController {
  ...
  Future<void> signInWithLinkedin(BuildContext context) async {
    try {
      ...
      SocialiteCredentials linkedinCredential =
          repo.apiClient.getSocialCredentialsConfigData();
      ...
      SignInWithLinkedIn.signIn(
        ...
        onGetUserProfile: (token, user) async {
          ...
          await socialLoginUser(
              provider: 'linkedin', accessToken: token.accessToken ?? '');
        },
        onSignInError: (error) {
  ...
  bool checkSocialAuthActiveOrNot({String provider = 'all'}) {
    if (provider == 'google') {
      return repo.apiClient.getSocialCredentialsConfigData().google?.status ==
          '1';
    } else if (provider == 'facebook') {
      return repo.apiClient.getSocialCredentialsConfigData().facebook?.status ==
          '1';
    } else if (provider == 'linkedin') {
      return repo.apiClient.getSocialCredentialsConfigData().linkedin?.status ==
          '1';
    } else {
