...
/// See also:
///
///  * [showCustomDateRangePicker], which shows a Material Design date range picker
        ...
        children: <Widget>[
          TextButton(
            ...
            child: Text(widget.cancelText ??
                (useMaterial3
                    ? localizations.cancelButtonLabel.tr
                    : localizations.cancelButtonLabel.toUpperCase().tr)),
          ),
          TextButton(
            onPressed: _handleOk,
            child:
                Text(widget.confirmText?.tr ?? localizations.okButtonLabel.tr),
          ),
        ],
      ),
    );
...
int daysName(DateTime? startDate) {
  return startDate != null ? startDate.weekday : 1;
}

/// A Material-style date range picker dialog.
///
/// It is used internally by [showCustomDateRangePicker] or can be directly pushed
/// onto the [Navigator] stack to enable state restoration. See
/// [showCustomDateRangePicker] for a state restoration app example.
///
/// See also:
///
///  * [showCustomDateRangePicker], which is a way to display the date picker.
...
class _CalendarRangePickerDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String endDateText = _formatRangeEndDate(
        localizations, selectedStartDate, selectedEndDate, DateTime.now());
    final int startWeekDayNumber = daysName(selectedStartDate);
    final int endWeekDayNumber = daysName(selectedEndDate);
    final String endDaysName = _formatRangeEndDate(
        localizations, selectedEndDate, selectedEndDate, DateTime.now());
    ...
    return SafeArea(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          leading: IconButton(
            onPressed: () {
              Get.find<HomeController>().setIsClickCheckOut(false);
              Get.back();
            },
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        Get.find<HomeController>().isClickCheckOut
                            ? Get.find<HomeController>().checkInDate
                            : startDateText,
                        style: boldLarge,
                      )
                    ],
                  ...
                  SvgPicture.asset(
                    ...
                    colorFilter: const ColorFilter.mode(
                        MyColor.titleColor, BlendMode.srcIn),
                  ),
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      // Text(Get.find<HomeController>().isClickCheckOut ? startDateText : endDateText,style: boldLarge,)
                      Text(
                        endDateText,
                        style: boldLarge,
                      )
              ...
              RoundedButton(
                ...
                text: MyStrings.applyDates.tr,
                press: () {
                  var controller = Get.find<HomeController>();

                  String startDays = getDayNameFromWeekday(startWeekDayNumber);
                  String endDays = getDayNameFromWeekday(endWeekDayNumber);
                  if (controller.isClickCheckOut) {
                    endDateText = endDateText;
                    controller.setCheckOutDate(startDateText, startDays);
                  } else {
                    controller.setCheckInCheckOutDate(
                        startDateText, endDateText, startDays, endDays);
                  }

                  controller.setIsClickCheckOut(false);
                  controller.setCheckOutFirstClickStatus(true);
                  Get.back();
                },
              )
            ],
...
const Duration _monthScrollDuration = Duration(milliseconds: 500);
  ...
  Widget _buildMonthItem(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color: MyColor.colorWhite, borderRadius: BorderRadius.circular(8)),
      child: _MonthItem(
  ...
  List<Widget> _getDayHeaders(
    ...
    for (int i = localizations.firstDayOfWeekIndex; true; i = (i + 1) % 7) {
      ...
      result.add(ExcludeSemantics(
        child: Center(child: Text(weekday.tr, style: headerStyle)),
      ));
      ...
      child: Center(
        child: Semantics(
          ...
          child: ExcludeSemantics(
            child: Text(
              localizations.formatDecimal(day).tr,
    ...
    return Column(
      children: <Widget>[
        Container(
          ...
          child: ExcludeSemantics(
            child: Text(
              localizations.formatMonthYear(widget.displayedMonth).tr,
    ...
    final Widget actions = Container(
      ...
      child: OverflowBar(
        ...
        children: <Widget>[
          ...
          TextButton(
            onPressed: onConfirm,
            child: Text(confirmText?.tr ?? localizations.okButtonLabel.tr),
          ),
        ],
      ),
    );
